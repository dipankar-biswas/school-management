        ...
        <tbody>
          @foreach($navbar as $data)
            <tr>
                ...
                <td>{{$data->name}}</td>
                <td>
                  @if(App\Models\navbar::where("id", $data->parentid)->count() > 0)
                    {{App\Models\navbar::where("id", $data->parentid)->first()->name}}
                  @else
                    N/A
                  @endif
                </td>
                ...
                <td>
                  <a class="btn btn-sm btn-primary" onclick="modalShowUpdate({{$data->id}})" href=""><i class="fa fa-edit"></i></a>
                    ...
                    <a onclick="return confirm('Are you sure Delete?')" href="{{Route('navbardelete.store', 'id='.$data->id)}}" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>
                </td>
            </tr>
          @endforeach
        </tbody>
        ...
        <form id="myForm" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              <select class="form-control" name="parentid">
                ...
                @if(isset($navbarsub) && $navbarsub != null)
                  @foreach($navbarsub as $navbarsdata)
                    <option value="{{$navbarsdata->id}}">{{$navbarsdata->name}}</option>
                  @endforeach
                @endif
              </select>
              <small class="form-text" id="errparentid"></small>
          </div>
          <div class="form-group">
...
<div class="modal fade" id="exampleModalUpdate" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update Navbar</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ...
        <form id="myFormUpdate" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              ...
              <small class="form-text" id="erruparentid"></small>
          </div>
      ...
      $.ajax({
            ...
            success: function(response){
                if(response.status == "getSelectdata"){
                    ...
                    $("#parentid").html(`<option value="0">Select</option>`);
                    $.each(response.navbarsub, function(key, value){
