    ...
    <div class="m-2 my-4">
      ...
      <div class="card-body">
          ...
          <div style="overflow: hidden;">
            <h3 class="float-left">Sidebar Link List</h3>
      ...
      @if($sidelink != null)
        <tbody>
          @foreach($sidelink as $data)
            <tr>
                <td>{{$loop->index +1}}</td>
                <td>{{$data->name}}</td>
                <td>{{$data->sidelinktitle?->name}}</td>
                <td>
                  <a href="{{$data->targetlink}}">View</a>
                </td>
                <td>
                  @if($data->status == 1)
                ...
                <td>
                  <a class="btn btn-sm btn-primary" onclick="modalShowUpdate({{$data->id}})" href=""><i class="fa fa-edit"></i></a>
                    ...
                    <a onclick="return confirm('Are you sure Delete?')" href="{{Route('sldelete.delete', 'id='.$data->id)}}" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="myForm" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              <input type="text" name="targetlink" class="form-control" placeholder="Target link*">
              <small class="form-text" id="errtargetlink"></small>
          </div>
          <div class="form-group">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="myFormUpdate" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              <input type="text" name="targetlink" id="targetlink" class="form-control" placeholder="Target link*">
              <small class="form-text" id="errutargetlink"></small>
          </div>
          <div class="form-group">
      ...
      $.ajax({
            ...
            success: function(response){
                if(response.status == "getSelectdata"){
                    ...
                    $("#targetlink").val(response.data.targetlink);
